...
class KalmanFilter:
    ...
    def update(self):
        self.K = self.P.dot(self.H.transpose()).dot(np.linalg.inv(
            self.H.dot(self.P).dot(self.H.transpose()) + self.R))
#         print("K_prime.shape:", K_prime.shape)
    ...
    def adaptive_RQ(self):
        ...
        self.R = (1 - self.alpha) * residual @ residual.transpose() + \
            self.H @ self.P_ @ self.H.transpose() + self.alpha * self.R
        innovation = (self.Z - self.H @ self.X_).reshape(-1, 1)
        self.Q = self.alpha * self.Q + \
            (1 - self.alpha) * \
            self.K @ innovation @ innovation.transpose() @ self.K.transpose()
